...
export function getImportDependenceByState() {
  ...
  const stateCoords = {
    'Lagos': { lat: 6.5244, lng: 3.3792, isPort: true },
    'Kano': { lat: 12.0022, lng: 8.5919, isPort: false },
    'Abuja': { lat: 9.0765, lng: 7.3986, isPort: false },
    'Borno': { lat: 11.8333, lng: 13.1500, isPort: false },
    'Rivers': { lat: 4.8396, lng: 6.9112, isPort: true }
  };
...
export function getInfrastructureQualityByState() {
  ...
  Object.keys(povertyData).forEach(state => {
    ...
    // Estimate components from HDI (simplified)
    const educationIndex = hdi * 0.95 + (Math.random() * 0.05 - 0.025); // Slight variation
    const healthIndex = hdi * 0.98 + (Math.random() * 0.04 - 0.02);
    const incomeIndex = hdi * 0.97 + (Math.random() * 0.06 - 0.03);
