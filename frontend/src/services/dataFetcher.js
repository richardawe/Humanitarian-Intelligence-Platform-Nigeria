...
// Note: This API should support CORS, if not, we may need a CORS proxy
...
const cache = {
  get(key) {
    try {
      const item = localStorage.getItem(`hip_cache_${key}`);
      if (!item) return null;
      const { data, timestamp } = JSON.parse(item);
      if (Date.now() - timestamp > CACHE_TTL) {
        localStorage.removeItem(`hip_cache_${key}`);
        return null;
      }
      return data;
    } catch (error) {
  ...
  set(key, data) {
    try {
      localStorage.setItem(`hip_cache_${key}`, JSON.stringify({
        data,
        timestamp: Date.now()
      }));
    } catch (error) {
 ...
 * If CORS issues occur, may need to use a CORS proxy
 */
export async function fetchExchangeRates() {
  // Check cache first
  const cached = cache.get('exchangeRates');
  if (cached) return cached;
  ...
  try {
    ...
    try {
      const cachedItem = localStorage.getItem('hip_cache_exchangeRates');
      if (cachedItem) {
...
export async function fetchHumanitarianData(state = null) {
  // Check cache first
  const cacheKey = state ? `humanitarian_${state}` : 'humanitarian_all';
  ...
  try {
    ...
    if (state) {
      ...
      cache.set(cacheKey, stateData);
      return stateData;
    }

    cache.set(cacheKey, humanitarianData);
    ...
    if (state) {
      const stateData = humanitarianData.find(item => item.state === state);
      const result = stateData || { error: `No data available for ${state}` };
